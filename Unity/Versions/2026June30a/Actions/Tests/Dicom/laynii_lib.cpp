...
{
    std::string datatype = nifti_datatype_to_string(nii->datatype); 

    int64_t ntot = nifti_get_volsize(nii);

    // Print nifti descriptives to command line for debugging
    cout << "    File name: " << nii->fname << endl;
    cout << "    Data size: " << nii->nvox <<" ("<< ntot <<") bytes"<< endl;
    cout << "    Image details: " << nii->nx  << " X | " << nii->ny << " Y | " << nii->nz << " Z | " << nii->nt << " T ("<< nii->nvox <<")  " << endl;
    cout << "    Voxel size = " << nii->pixdim[1] << " x " << nii->pixdim[2]   << " x " << nii->pixdim[3] << endl;
    cout << "    Datatype = " << nii->datatype << " (" << datatype << ")  \n" << endl;
}
