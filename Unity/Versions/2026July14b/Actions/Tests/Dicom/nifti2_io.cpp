                                 ...
                                 const int64_t * blist, nifti_brick_list * NBL)
{
   ...
   if( blist && nbricks <= 0 ){
      ...
      fprintf(stderr,"** nifti_image_read_bricks: bad nbricks,  \n");
      return NULL;
   }
                                               ...
                                               int64_t nbricks )
{
   ...
   if( g_opts.debug > 2 ){
      fprintf(stderr,"+d updating image dimensions for  bricks in list\n");
      fprintf(stderr,"   ndim =  \n");
      fprintf(stderr,"   nx,ny,nz,nt,nu,nv,nw: ( ,, , , , , )\n");
   }
                        ...
                        int64_t * sindex, nifti_brick_list * NBL, znzFile fp )
{
   ...
   if( !slist ){
      for( c = 0; c < NBL->nbricks; c++ ) {
         ...
         if( rv != NBL->bsize ){
            fprintf(stderr,"** NIFTI load bricks: cannot read brick    from \n");
            return -1;
         }
      }
      if( g_opts.debug > 1 )
         fprintf(stderr,"+d read   default     -byte bricks from file \n");
      return 0;
   }
       ...
       if( isrc != prev ){
          ...
          if( fposn != (oposn + isrc*NBL->bsize) ){
             ...
             if( znzseek(fp, fposn, SEEK_SET) < 0 ){
                fprintf(stderr,"** NIFTI: failed to locate brick     in file \n");
                return -1;
             }
          }
          ...
          if( rv != NBL->bsize ){
             fprintf(stderr,"** NIFTI: failed to read brick     from file \n");
             if( g_opts.debug > 1 )
                fprintf(stderr,"   (read   of   bytes)\n");
             return -1;
          }
          fposn += NBL->bsize;
       } else {
                               ...
                               nifti_brick_list * nbl)
{
   ...
   if( ! nbl->bricks ){
     fprintf(stderr,"** NIFTI NANM: failed to alloc void ptrs\n");
     return -1;
   }
   ...
   for( c = 0; c < nbl->nbricks; c++ ){
      ...
      if( ! nbl->bricks[c] ){
         fprintf(stderr,"** NIFTI NANM: failed to alloc      bytes for brick  \n");
   ...
   if( g_opts.debug > 2 )
      fprintf(stderr,"+d NANM: alloc'd   bricks of     bytes for NBL\n");
                           ...
                           int64_t ** slist, int64_t ** sindex)
{
   ...
   *sindex = (int64_t *)malloc(nbricks * sizeof(int64_t));
   ...
   if( !*slist || !*sindex ){
      fprintf(stderr,"** NIFTI NCS: failed to alloc     ints for sorting\n");
   ...
   if( g_opts.debug > 2 ){
      ...
      fprintf(stderr,  "  orig   : ");
      for( c1 = 0; c1 < nbricks; c1++ ) fprintf(stderr," ");
      fprintf(stderr,"\n  new    : ");
      for( c1 = 0; c1 < nbricks; c1++ ) fprintf(stderr," ");
      fprintf(stderr,"\n  indices: ");
      for( c1 = 0; c1 < nbricks; c1++ ) fprintf(stderr," ");
                           ...
                           const int64_t * blist, int disp_error)
{
   ...
   if( nim->dim[0] < 3 ){
      if( disp_error || g_opts.debug > 1 )
        fprintf(stderr,"** NIFTI: cannot read explicit brick list from -D dataset\n");
      return 0;
   }
   ...
   if( nsubs <= 0 ){
      fprintf(stderr,"** NIFTI VNBL warning: bad dim list ( ,, , )\n");
      return 0;
   }
                                 ...
                                 const nifti_brick_list *NBL)
{
   ...
   if( volbytes != NBL->bsize ) {
      if( g_opts.debug > 1 )
         fprintf(stderr,"** NIFTI NBL/nim mismatch, volbytes =    ,  \n");
      errs++;
   }

   if( nvols != NBL->nbricks ) {
      if( g_opts.debug > 1 )
         fprintf(stderr,"** NIFTI NBL/nim mismatch, nvols =    ,  \n");
      errs++;
   }

   if( errs ) return 0;
   else if ( g_opts.debug > 2 )
      fprintf(stderr,"-- nim/NBL agree: nvols =    , nbytes =  \n");
...
char * nifti_findhdrname(const char* fname)
{
   ...
   if (nifti_fileexists(hdrname)) {
      free(basename); 
      /*Chris Rorden 2020, for FSL compatibility do not allow "img.nii" and "img.nii.gz"*/
                         ...
                         int set_byte_order )
{
   ...
   if( nim->fname ) 
      ...
      //nim->iname = nifti_makeimgname(nim->fname, nim->nifti_type, check, comp); //chris rorden - fsl compat
...
int disp_nifti_2_header( const char * info, const nifti_2_header * hp )
{
   ...
   fprintf(fp, "    slice_start    =  \n");
   fprintf(fp, "    slice_end      =  \n");
...
int nifti_hdr2_looks_good(const nifti_2_header * hdr)
{
   ...
   if( d0 < 0 || d0 > 7 ) {
      if( g_opts.debug > 0 )
         fprintf(stderr,"** NIFTI: bad n2hdr: dim0 =  \n");
      errs++;
   } else { /* only check dims if d0 is okay */
      for( c = 1; c <= d0; c++ )
         if( hdr->dim[c] <= 0 ){
           if( g_opts.debug > 0 )
             fprintf(stderr,"** NIFTI: bad nhdr field: dim[] =  \n");
           errs++;
         }
   }
...
static int nifti_read_extensions( nifti_image *nim, znzFile fp, int64_t remain )
{
   ...
   if( g_opts.debug > 2 )
      fprintf(stderr,"-d nre: posn= , offset=   , type=, remain= \n");
   ...
   if( remain < 16 ){
      if( g_opts.debug > 2 ){
         if( g_opts.skip_blank_ext )
            ...
            fprintf(stderr,"-d remain= , no space for extensions\n");
      }
      return 0;
   }
   ...
   remain -= 4;
   if( g_opts.debug > 2 )
      fprintf(stderr,"-d found valid 4-byte extender, remain =  \n");
   ...
   while (nifti_read_next_extension(&extn, nim, remain, fp) > 0)
   {
      if( nifti_add_exten_to_list(&extn, &Elist, (int)count+1) < 0 ){
         free(Elist);
         if( g_opts.debug > 0 )
           fprintf(stderr,"** NIFTI: failed adding ext   to list\n");
         return -1;
      }

      /* we have a new extension */
      if( g_opts.debug > 1 ){
         fprintf(stderr,"+d found extension #   , code = ");
   ...
   if( g_opts.debug > 2 )
      fprintf(stderr,"+d found   extension(s)\n");
...
static znzFile nifti_image_load_prep( nifti_image *nim )
{
   ...
   if( znzseek(fp , (long)ioff , SEEK_SET) < 0 ){
      fprintf(stderr,"** NIFTI: could not seek to offset     in file \n");
      znzclose(fp);
      return NULL;
   }
                                ...
                                nifti_image *nim)
{
  ...
  if( ii < ntot ){
    if( g_opts.debug > 0 )
       fprintf(stderr,"++ WARNING: nifti_read_buffer():\n") ;
    /* memset( (char *)(dataptr)+ii , 0 , ntot-ii ) ;  now failure [rickr] */
    return -1 ;
  }

  if( g_opts.debug > 2 )
    fprintf(stderr,"+d nifti_read_buffer: read   bytes\n");
...
#ifdef isfiniteX /* Chris Rorden, 2020: disable isfinite to preserve NaN */
{
                         ...
                         const nifti_brick_list * NBL)
{
   ...
   if( !NBL ){ /* just write one buffer and get out of here */
      ...
      if (ss < nim->nbyper * nim->nvox){
         fprintf(stderr,
            "** NIFTI ERROR (NWAD): wrote only   of  bytes to file\n");
         return -1;
      }

      if( g_opts.debug > 1 )
         fprintf(stderr,"+d wrote single image of   bytes\n");
   } else {
      if( ! NBL->bricks || NBL->nbricks <= 0 || NBL->bsize <= 0 ){
         fprintf(stderr,"** NIFTI error (NWAD): no brick data to write (,, )\n");
         return -1;
      }
      ...
      for( bnum = 0; bnum < NBL->nbricks; bnum++ ){
         ...
         if( ss < NBL->bsize ){
            fprintf(stderr,
            "** NIFTI ERROR (NWAD): wrote only   of  bytes of brick   of   to file\n");
            return -1;
         }
      }
      if( g_opts.debug > 1 )
         fprintf(stderr,"+d wrote image of      brick(s), each of   bytes\n");
   }
                                          ...
                                          int arg_dtype)
{
   ...
   if( dim[0] < 1 || dim[0] > 7 ) {
      fprintf(stderr,"** nifti_simple_hdr_with_dims: bad dim[0]= \n");
      dim = default_dims;
   } else {
      for( c = 1; c <= dim[0]; c++ )
         if( dim[c] < 1 )
         {
            fprintf(stderr,
                "** nifti_simple_hdr_with_dims: bad dim[%d]= \n",
                c);
            dim = default_dims;
            break;
         }
                                          ...
                                          int arg_dtype)
{
   ...
   if( dim[0] < 1 || dim[0] > 7 ) {
      ...
      for( c = 1; c <= dim[0]; c++ )
         ...
         {
            fprintf(stderr,
                "** nifti_simple_hdr_with_dims: bad dim[]= \n");
            dim = default_dims;
            break;
         }
   }
   ...
   if( g_opts.debug > 1 )
      fprintf(stderr,"+d make_new_n1_header, dim[0] =    , datatype = %d\n",dtype);
                                 ...
                                 int data_fill)
{
   ...
   if( data_fill ) {
      ...
      if( !nim->data ) {
         fprintf(stderr,"** NIFTI NMNN: failed to alloc     bytes for data\n");
         nifti_image_free(nim);
         nim = NULL;
      }
   }
...
#define N_CHECK_2BYTE_VAL(fn) do { if( ! NIFTI_IS_16_BIT_INT(nim->fn) ) { \
   fprintf(stderr,"** nim-> = does not fit into NIFTI-1 header\n"); return 1; } } while(0)
               ...
               const char * opts, znzFile imgfile, const nifti_brick_list * NBL)
{
   ...
   if( g_opts.debug > 1 ){
      ...
      if( g_opts.debug > 2 )
         fprintf(stderr,"-d nifti type %d, offset  \n",
                 nim->nifti_type);
   }
   ...
   */
   /*Chris Rorden - use environment to detect NIfTI2
      https://fsl.fmrib.ox.ac.uk/fsl/fslwiki/FslEnvironmentVariables
      NIFTI2_GZ, NIFTI2, NIFTI2_PAIR_GZ, NIFTI2_PAIR
      export FSLOUTPUTTYPE=NIFTI2
    */
   /*const char *key = "FSLOUTPUTTYPE";
   char   nver2Key[7] = "NIFTI2";
   char *value;
   value = getenv(key);
   if ((value != NULL) && strstr(value,nver2Key)  ) {*/
...
int nifti_nim_has_valid_dims(nifti_image * nim, int complain)
{
   ...
   if( nim->dim[0] <= 0 || nim->dim[0] > 7 ){
      errs++;
      if( complain )
        //fprintf(stderr,"** NIFTI NVd: dim[0] out of range [1,7]\n", nim->dim[0]);
      return 0;
   }
   ...
   if( nim->ndim != nim->dim[0] ){
      errs++;
      if( ! complain ) return 0;
      fprintf(stderr,"** NIFTI NVd: ndim != dim[0] ( , )\n");
   }
   ...
   if( ( (nim->dim[0] >= 1) && (nim->dim[1] != nim->nx) ) ||
      ...
      errs++;
      if( !complain ) return 0;
      fprintf(stderr,"** NIFTI NVd mismatch: dims    =  ,   , , , , , \n"
              "                 nxyz... =  , ,   , , , , \n");
   }

   if( g_opts.debug > 2 ){
      fprintf(stderr,"-d check dim[ ] =");
      for( c = 0; c < 7; c++ ) fprintf(stderr,"  ");
      fputc('\n', stderr);
   }
   ...
   for( c = 1; c <= nim->dim[0]; c++ ){
      if( nim->dim[c] > 0)
         ...
         if( !complain ) return 0;
         fprintf(stderr,"** NIFTI NVd: dim[ ] (= ) <= 0\n");
         errs++;
      }
   }
   if( prod != nim->nvox ){
      if( ! complain ) return 0;
      fprintf(stderr,"** NIFTI NVd: nvox does not match    -dim product ( ,  )\n");
      errs++;
   }
   ...
   if( g_opts.debug > 1 )
      for( c = nim->dim[0]+1; c <= 7; c++ )
         if( nim->dim[c] != 0 && nim->dim[c] != 1 )
            fprintf(stderr,"** NIFTI NVd warning: dim[ ] =   , but ndim =  \n");
                                ...
                                void ** data )
{
   ...
   for( c = 1; c <= nim->dim[0]; c++ ){
      if( dims[c] >= nim->dim[c] ){
         fprintf(stderr,"** nifti_RCI: dims[ ] >= nim->dim[    ] ( , )\n");
         return -1;
      }
   }
   ...
   if( g_opts.debug > 1 )
      fprintf(stderr,"+d read   bytes of collapsed image from %s\n",
              nim->fname);
                         ...
                         znzFile fp, int64_t base_offset)
{
   ...
   if( nprods == 1 ){
      ...
      if( nread != bytes ){
         fprintf(stderr,"** NIFTI rciRD: read only   of      bytes from \n");
         return -1;
      } else if( g_opts.debug > 3 )
         fprintf(stderr,"+d successful read of      bytes at offset  \n");
   ...
   for( c = 0; c < prods[0]; c++ ){
      ...
      if( g_opts.debug > 3 )
         fprintf(stderr,"-d reading   bytes, foff      +  , doff  \n");
...
static int rci_alloc_mem(void **data, int64_t prods[8], int nprods, int nbyper )
{
   ...
   if( ! *data ){   /* then allocate what is needed */
      if( g_opts.debug > 1 )
         fprintf(stderr,"+d alloc      (  x ) bytes for collapsed image\n");

      *data = malloc(size);   /* actually allocate the memory */
      if( ! *data ){
        fprintf(stderr,"** NIFTI rci_am: failed to alloc  bytes for data\n");
        return -1;
      }
   } else if( g_opts.debug > 1 )
      fprintf(stderr,"-d rci_am: *data already set, need     x  bytes\n");
...
int64_t * nifti_get_int64list( int64_t nvals , const char * str )
{
   ...
   if( g_opts.debug > 1 )
      fprintf(stderr,"-d making int_list (vals =  ) from \n");
   ...
   while( ipos < slen && !ISEND(str[ipos]) ){
     ...
     while( isspace((int) str[ipos]) ) ipos++ ;   /* skip blanks */
      ...
      if( str[ipos] == '$' ){  /* special case */
         ...
         if( ibot < 0 ){
           fprintf(stderr,"** NIFTI ERROR: list index is out of range 0.. \n") ;
         ...
         nused = (cpt-(str+ipos)) ;
         if( ibot == 0 && nused == 0 ){
           fprintf(stderr,"** NIFTI : list syntax error \n") ;
           free(subv) ; return NULL ;
         }
         ipos += nused ;
      }
      ...
      if( str[ipos] == ',' || ISEND(str[ipos]) ){
         ...
         if( !subv_realloc ) {
            free(subv);
            fprintf(stderr,"** nifti_get_intlist: failed realloc of    ints\n");
            return NULL;
         }
         subv = subv_realloc;
         subv[0]    = nout ;
      ...
      if( str[ipos] == '-' ){
         ...
         fprintf(stderr,"** NIFTI ERROR: index list syntax is bad: '\n") ;
         free(subv) ; return NULL ;
      }
      ...
      if( str[ipos] == '$' ){  /* special case */
         ...
         if( itop < 0 ){
           fprintf(stderr,"** NIFTI ERROR: index is out of range 0.. \n") ;
           free(subv) ; return NULL ;
         }
         if( itop >= nvals ){
           fprintf(stderr,"** NIFTI ERROR: index  is out of range 0.. \n") ;
      ...
      if( str[ipos] == '(' ){  /* decode an integer */
        ...
        fprintf(stderr,"** NIFTI WARNING: index list ' ..( )' means nothing\n" );
         }
      }
   ...
   if( g_opts.debug > 1 ) {
      fprintf(stderr,"+d int_list (vals =  ): ");
...
int * nifti_get_intlist( int nvals , const char * str )
{
   ...
   if( nints > INT_MAX ) {
      fprintf(stderr,"** nifti_get_intlist: ints is too long for 32-bits\n");
      free(i64list);
      return NULL;
   }
   ...
   if( !ilist ) {
      fprintf(stderr,"** nifti_get_intlist: failed to alloc  ints\n"); 
      free(i64list);
      return NULL;
   }
   ...
   for( index=0; index <= nints; index++ ) {
      if( i64list[index] > INT_MAX ) {
         fprintf(stderr,"** nifti_get_intlist: value  too big for 32-bits\n");
         free(ilist);
         free(i64list);
         return NULL;
      }
