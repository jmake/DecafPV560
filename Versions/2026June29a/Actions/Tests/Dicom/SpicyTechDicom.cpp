...
*/
    ...
    //int VtkLoader::LoadFile(std::string filename, std::string key)
    int VtkManager::SetObj(vtkDataObject* obj, std::string key)
    {
        //obj = PReaderSerial(filename); 
        if(!obj) return 0; 
    ...
    {
      if(obj) PWriterSerial(obj, fname); 
    }
