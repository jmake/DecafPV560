...
std::cout << "=======================================================================================\n"; 
    ...
    {
        if(vti == nullptr) return false; 

        std::vector<double>  right = {r[0],r[1],r[2]};  // X axis 
        std::vector<double>     up = {u[0],u[1],u[2]};  // Y axis 
        std::vector<double> normal = {n[0],n[1],n[2]};  // Z axis 
        std::vector<double> origin = {o[0],o[1],o[2]};

        vtkDataObject *cutter3 = CutterPlane4(
                                                vti, 
                                                right, 
                                                up,
                                                normal,
        ...
        //SetOrigin(std::abs(n[0]) * origin[0], std::abs(n[1]) * origin[1], std::abs(n[2]) * origin[2]); 
        return true; 
    }


    void CutterManager2::SetOrigin(double x, double y, double z) 
    {
        auto image = vtkImageData::SafeDownCast(obj);  
    ...
    void CutterManager2::GetOrigin(double* origin) 
    {
        ...
        std::copy(array.data(), array.data() + array.size(), origin);  
    }
