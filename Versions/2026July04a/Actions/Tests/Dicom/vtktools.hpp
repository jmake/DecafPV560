...
std::vector<int> GetFlatCellIndices(vtkDataObject* obj, int& nCells)
{
    ...
    {
        nCells = 0; 
        std::cerr << "[GetFlatCellIndices] Error: input is not '??'.\n";
        return flatIndices;
    }
...
vtkDataObject* CenterAtOrigin(vtkDataObject* obj)
{
    ...
    if (auto data = vtkDataSet::SafeDownCast(obj)) {
        double bounds[6];
...
//--------------------------------------------------------------------------||--//
void ArrayGet(vtkPointData* pd, std::string key, std::vector<float>& range, std::vector<float>& property)
{
