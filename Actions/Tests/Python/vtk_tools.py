...
## paraview.simple *; GetVtkObjFromParaview = lambda _pvObj : servermanager.Fetch(_pvObj)
...
def CreateMovie(plotsName, movieName):
  ...
  ## "-r 50" and "setpts=0.25*PTS" accelerate out.mp4   
  time ffmpeg -r 50 -pattern_type glob -i '*.png' -c:v libx264 -vf fps=100 -filter:v "setpts=0.25*PTS" -pix_fmt yuv420p out.mp4
...
def Vtx2Vtu( vtkObj ) :  
  ...
  appendFilter.AddInputData( vtkObj )   
  appendFilter.Update() 
...
#--------------------------------------------------------------------------||--#    
def GetLineSource(p1,p2,nPts):
    line = vtk.vtkLineSource()
    line.SetResolution(nPts)
    line.SetPoint1(p1)
    line.SetPoint2(p2)
    line.Update()
    return line.GetOutputDataObject(0) 

"""
def GetSphereSource> sphere;
...
#--------------------------------------------------------------------------||--#    
def GetSimpleVTS(X,Y,Z, Dims, Ws={}, fname=""):
  Mesh     = vtk.vtkStructuredGrid()
  Mesh.SetDimensions(Dims) #(Ny,Nx,1))

  Points   = vtk.vtkPoints()
  Vertices = vtk.vtkCellArray()

  for pt in zip(X,Y,Z) :
    idx  = Points.InsertNextPoint(pt)
    vert = vtk.vtkVertex()
  ...
  for k,v in Ws.items() :
    vtk_array = GetVtkArray(v,k)
    Mesh.GetPointData().AddArray(vtk_array)

  if len(fname) : Writer(Mesh,fname)
  return Mesh
...
def WriteAsObj( vtp, fname ):
  writer = vtk.vtkOBJWriter();
  writer.SetInputData( vtp );
  writer.SetFileName(fname  + ".obj") #"%s.%s" % (fname,writer.GetDefaultFileExtension()) )
  writer.Update();
  #print("\t[Writer] '%s' Saved!!" % )
  print("\t[Writer] '%s' Saved!!" %(writer.GetFileName()) )



def GetOBJExporter( vtp, fname ):  
  exporter = vtk.vtkOBJExporter();
  exporter.SetFilePrefix(fname  + ".obj");
  exporter.SetInputData( vtp );
  exporter.Write();
...
def GetFieldDataArray(vtkObj, key=None):
  with np.printoptions(precision=3, suppress=True):
    ...
    NumberOfArrays = cellData.GetNumberOfArrays()
    ArraysName = [cellData.GetArrayName(iarray) for iarray in range(NumberOfArrays)]
    if not NumberOfArrays : return Array 
...
#--------------------------------------------------------------------------||--#
class PolyDataReactJS : 
  def __init__(self, vtp, fname, prop, Scale=1.0) :

    Coords         = GetCoords(vtp) * Scale
    W              = GetPointData(vtp.GetPointData())[prop] #["W"]; #print( W.shape )
    Vertices,Types = GetCellsList(vtp); #print(Vertices); exit() 

    print("\t[PolyDataReactJS] '%s':[%f:%f]"%(prop,W.min(),W.max()) )

    POINTS = Coords.flatten()
    POINTS = ",".join([str(p) for p in POINTS]); #print( POINTS ); exit()  

    POLYS     = [ [len(v)] + v.tolist() for v in Vertices] 
    POLYS     = [ p for P in POLYS for p in P]  
    POLYS     = ",".join([str(p) for p in POLYS]); #print( POLYS ); exit() 

    NAMEARRAY = "Pressure"
    DATAARRAY = W 
    DATAARRAY = ",".join([str(p) for p in W]); #print(DATAARRAY)
    ...
    DATAARRAY = self.SetDataArray(DATAARRAY, NAMEARRAY,"    ")     
    POINTDATA = self.SetPointData(DATAARRAY) 
    POLYDATA  = self.SetPolyData(POINTS,POLYS, POINTDATA) ; #print( POLYDATA )
...
import { PointData } from 'react-vtk-js';
import { DataArray } from 'react-vtk-js';

function VTK_NAME_COMPONENT() 
{
  return (
    <div style={{width: '50vw', height: '50vh'}}>
      <View id="0" background={[0.0, 0.0, 0.0]}>
        <GeometryRepresentation
                  mapper={{colorByArrayName: 'Pressure', scalarMode: 3, interpolateScalarsBeforeMapping: true, isOpaque:true, mapScalars:['Pressure',0.1] }}
                  colorDataRange={[%f,%f]}
...
export default VTK_NAME_COMPONENT; 
    """ 

    fout = open(fname, "w")
    fout.write( JAVASCRIPT % (W.min(), W.max(), POLYDATA) )
    fout.close() 
    return 


  def SetPolyData(self, POINTS, POLYS, POINTDATA):
    TEXT  = "<PolyData \n"
    TEXT += "  points={[%s]} \n" % POINTS 
    TEXT += "  polys={[%s]} \n" % POLYS 
    TEXT += "> \n"  
  ...
  def SetPointData(self, POINTDATA, TAP="  ") : 
    TEXT  = "%s<PointData>  \n" % TAP 
    TEXT += "%s \n" % (POINTDATA) 
    TEXT += "%s</PointData>  \n" % TAP
    return TEXT  
  ...
  def SetDataArray(self, ARRAY_DATA, ARRAY_NAME, TAP="") : 
    TEXT  = "TAP<DataArray \n"
    TEXT += "TAP  name=\"%s\" \n" % ARRAY_NAME 
    TEXT += "TAP  values={[%s]} \n" % ARRAY_DATA  
    TEXT += "TAP/> " 
    return TEXT.replace("TAP",TAP) 
...
def GetThreshold(vtkObj, key, greaterthat=None, lessthat=None) : 
  t = vtk.vtkThreshold()  
  ...
  if( (lessthat != None) and (greaterthat == None) ) : 
    t.ThresholdByLower(lessthat   ) #  less or equal 
  if( (lessthat == None) and (greaterthat != None) ) :
    t.ThresholdByUpper(greaterthat) #   greater or equal  
  if( (lessthat != None) and (greaterthat != None) ) :
    t.ThresholdBetween(greaterthat, lessthat)
...
def AppendVtusDic( vtkObjDic ):
  append = vtk.vtkAppendFilter()
##for k,vtkObj in vtkObjDic.items() : append.AddInputData(vtkObj)
...
#--------------------------------------------------------------------------||--#
def GetPointDataToCellData( vtkObj, key ):  
  a = vtk.vtkPointDataToCellData() 
  a.SetInputData( vtkObj )  
  a.SetInputArrayToProcess( 0, 0, 0, 0, key)
  a.SetPassPointData(True)
  a.Update()
...
#--------------------------------------------------------------------------||--#
def GetCellDataToPointData( vtkObj, key=""):
  a = vtk.vtkCellDataToPointData()
  a.SetInputData( vtkObj )
  #a.SetInputArrayToProcess( 0, 0, 0, 0, key)
  a.SetPassCellData(True)
  a.Update()
  return a.GetOutputDataObject(0)

#  Array = GetDataArray(a.GetOutputDataObject(0), key)
#  return Array
...
class __GetPolyDataVertex :
  ...
  def SetVertices(self, Pts, Ids=[]) :  
    ...
    #self.Mesh.SetLines(self.PolyLine)  # <- PolyLine  
...
class GetPolyDataVertexSimple :
  ...
  def __init__(self, time=-1):
    ...
    self.Mesh = vtk.vtkPolyData()
    if time >= 0 :
      SetFieldData(self.Mesh, np.array([time]), "TimeValue")
      self.Mesh.GetInformation().Set(vtk.vtkDataObject.DATA_TIME_STEP(), time ) # ?? 
    return
...
#--------------------------------------------------------------------------||--#
class GetSimpleStructuredGrid :
  def __init__(self, dims=[], time=-1):
    self.Points    = None
    self.Time      = None

    self.Mesh = vtk.vtkStructuredGrid()
    if len(dims) : self.Mesh.SetDimensions( dims );
    return


  def SetVertices(self, X, Y, Z=[], Ids=[]) :
    self.Points   = vtk.vtkPoints()
    self.Vertices = vtk.vtkCellArray()

    if not len(Z) : Z = np.zeros( len(X) )   
    for pt in zip(X,Y,Z) :
      ## X.1. ---
      idx = self.Points.InsertNextPoint(pt)
      vert = vtk.vtkVertex()
    ...
    assert(np.prod(self.Mesh.GetDimensions()) == len(X))
    return
...
#--------------------------------------------------------------------------||--#
class GetSimpleUnstructuredGrid :
  def __init__(self, dims=[], time=-1):
    self.Points    = None
    self.Time      = None
  ...
  def SetTetras(self, Connectivity): 
    for row in Connectivity : 
      assert len(row)==4 
      Ids = vtk.vtkIdList()  
      [Ids.InsertNextId(row[i]) for i in range(len(row))] 
      self.Mesh.InsertNextCell(vtk.VTK_TETRA, Ids)    
    return 


  def SetPrisms(self, Connectivity):
    for row in Connectivity :
      assert len(row)==6
      Ids = vtk.vtkIdList()
      [Ids.InsertNextId(row[i]) for i in range(len(row))] 
      self.Mesh.InsertNextCell(vtk.VTK_WEDGE, Ids)
    return


  def SetHexas(self, Connectivity): 
    #self.SetAnyCells(Connectivity, vtk.vtkHexahedron(), 8)
    nVtkCellType = 8 
    vtkCellType  = vtk.vtkHexahedron() 

    cellArray = vtk.vtkCellArray()

    for row in Connectivity[-2:] :
      assert len(row)==nVtkCellType 
      Ids = vtk.vtkIdList()
      [Ids.InsertNextId(row[i]) for i in range(len(row))]

      cell = vtkCellType 
      cell = vtk.vtkHexahedron()  
      i=0; cell.GetPointIds().SetId(i,row[i])
      i=1; cell.GetPointIds().SetId(i,row[i])
      i=2; cell.GetPointIds().SetId(i,row[i])
      i=3; cell.GetPointIds().SetId(i,row[i])
      i=4; cell.GetPointIds().SetId(i,row[i])
      i=5; cell.GetPointIds().SetId(i,row[i])
      i=6; cell.GetPointIds().SetId(i,row[i])
      i=7; cell.GetPointIds().SetId(i,row[i])
      cellArray.InsertNextCell(cell)
      #self.Mesh.InsertNextCell(vtkCellType.GetCellType(), Ids)
    #self.Mesh.SetCells(vtkCellType.GetCellType(), cellArray)
    self.Mesh.SetCells(vtk.VTK_HEXAHEDRON, cellArray) 
    return


  def SetPyramids(self, Connectivity): 
    self.SetAnyCells(Connectivity, vtk.vtkPyramid(), 5)



  def SetAnyCells(self, Connectivity, vtkCellType, nVtkCellType):
    for row in Connectivity :
      assert len(row)==nVtkCellType 
      Ids = vtk.vtkIdList()
      [Ids.InsertNextId(row[i]) for i in range(len(row))]
  ...
  def Save(self, fin) :
    Writer(self.Mesh, fin, Ascii=True)



#--------------------------------------------------------------------------||--#
def GetFileSeriesWriter(TimeSeries, fname) :
  Base = """{
...
%s
  ...
  Text = ["{ \"name\" : \""+ serie[0] +"\", \"time\" : %e }" % serie[1] for serie in TimeSeries]  
  Text = ',\n'.join(Text)
  #print( Base % Text )

  fout = open(fname, "w")
  fout.write( Base % Text )
  fout.close() 
...
#--------------------------------------------------------------------------||--#
def SetPolyline(VtkPoints, Pts):
  PolyLine = vtk.vtkPolyLine()
  PolyLine.GetPointIds().SetNumberOfIds( len(Pts) )
  for ipt,pt in enumerate(Pts) :
    idx  = VtkPoints.InsertNextPoint(pt)
    PolyLine.GetPointIds().SetId(ipt,idx);
  return PolyLine


def SetVertex(VtkPoints, Pts):  
  Vert = vtk.vtkVertex()  
  idx  = self.Points.InsertNextPoint(pt)
  Vert.GetPointIds().SetId(0,idx)
  return Vert 
...
"""
      ...
      #Obj    = GetVtkObjFromParaview( input_meshpvtu )
      #pa = vtk.vtkPythonAlgorithm()
      #pa.SetPythonObject(MyAlgorithm())
      #pa.SetInputDataObject( Obj )
      #pa.Update()
"""
