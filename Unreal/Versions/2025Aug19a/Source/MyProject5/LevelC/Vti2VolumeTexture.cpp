...
//----------------------------------------------------------------------------------|  |--//
UVolumeTexture* CreateVolumeTextureFromBuffer(int32 Width, int32 Height, int32 Depth, const TArray<uint8>& VolumeBuffer) 
{
	if (VolumeBuffer.Num() != Width * Height * Depth * 4) return nullptr;

    UVolumeTexture* VolumeTex = UVolumeTexture::CreateTransient(Width, Height, Depth, PF_R8G8B8A8, NAME_None);
...
//----------------------------------------------------------------------------------|  |--//
UVolumeTexture* CreateVolumeTextureFromFloatData(int32 Width, int32 Height, int32 Depth, TArrayView<float> Data)
{
    ...
    // Step 2: allocate uint8 buffer for RGBA8
    TArray<uint8> VolumeBuffer;
    VolumeBuffer.SetNumUninitialized(Width * Height * Depth * 4);

    // Step 3: convert float -> RGBA8
    FLinearColor ColorMin = FLinearColor::White;  // or any mapping you want
    ...
    return CreateVolumeTextureFromBuffer(Width, Height, Depth, VolumeBuffer);
}
...
//CreateDataTest(dataTotal, widthLocal, heightLocal); 
	...
	SaveDynamicTexture2D(dynamicTexture, dynamicTextureName + ".png"); 

	UVolumeTexture* volumeTexture = CreateVolumeTextureFromFloatData(widthLocal, heightLocal, deepLocal, TArrayView<float>(dataTotal));	
