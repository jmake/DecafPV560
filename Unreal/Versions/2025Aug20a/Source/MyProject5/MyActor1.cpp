					...
					) 
{
	...
	UE_LOG(LogTemp, Warning, TEXT("[Vti2Surface] n_triangles = %d n_raw = %d"), n_triangles, n_raw / 4);
}
*/
...
void CutterManager::SetVti(vtkImageData* vti_ptr, UWorld* uworld) 
{
	...
	UE_LOG(LogTemp, Warning, TEXT("[CutterManager] VTI Dimensions : (%d,%d,%d)"), dimensions[0], dimensions[1], dimensions[2] );
}
									...
									) 
{
    ...
    Async(EAsyncExecution::Thread, [this, Promise, anatomicalView, x0, y0, z0]()
    {
    	...
    	Result.Buffer = CreateBuffer(anatomicalView, x0, y0, z0, Result.Width, Result.Height, Result.SpacingX, Result.SpacingY);
		UE_LOG(LogTemp, Warning, TEXT("[CreateTextureAsync1] Width: %d Height: %d SpX: %f SpY: %f"), Result.Width, Result.Height, Result.SpacingX, Result.SpacingY);
    ...
    Future.Next([this, &actor, name, After](const FCreateResult& Result)
    {
        ...
        {
        	UE_LOG(LogTemp, Warning, TEXT("[CreateTextureAsync2] Width: %d Height: %d SpX: %f SpY: %f"), Result.Width, Result.Height, Result.SpacingX, Result.SpacingY);
...
APixelTextureDisplay* CutterManager::CreateTexture(double anatomicalView[9], float x0, float y0, float z0, std::string fname) 
{
	...
	int width = -1, height = -1; 
	float spacingx = -1.0, spacingy = -1.0; 
	float* buffer = CreateBuffer(anatomicalView, x0, y0, z0, width, height, spacingx, spacingy); 
...
{
	double Axial[9] = {1, 0,  0, 0, 1,  0, 0, 0, 1}; 
	return CreateBuffer(Axial, x0, y0, z0, width, height, sizeX, sizeY); 
} 
...
{
	double Coronal[9] = {1, 0,  0, 0, 0, -1, 0, 1, 0};
	return CreateBuffer(Coronal, x0, y0, z0, width, height, sizeX, sizeY); 
} 
...
{
  	double Sagittal[9] = {0, 0, -1, 1, 0,  0, 0, 1, 0}; 
	return CreateBuffer(Sagittal, x0, y0, z0, width, height, sizeX, sizeY); 
} 


float* CutterManager::CreateBuffer(
									double anatomicalView[9], 
									float x0, float y0, float z0, 
									int& width, int& height, 
								...
								)
{
	...
	int dimensions2[3];
	domain->GetDimensions(dimensions2); 
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] VTI Dimensions : (%d,%d,%d)"), dimensions2[0], dimensions2[1], dimensions2[2] );

	double center[3]; 
	domain->GetCenter( center ); 
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] VTI Center : (%f,%f,%f)"), center[0], center[1], center[2] );
	...
	float x = NormalizeToRange(x0, bounds[0], bounds[1], vmin, vmax); 
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] x0 : %f -> %f"), x0, x);

	float y = NormalizeToRange(y0, bounds[2], bounds[3], vmin, vmax);
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] y0 : %f -> %f"), y0, y);

	float z = NormalizeToRange(z0, bounds[4], bounds[5], vmin, vmax);
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] z0 : %f -> %f"), z0, z);

	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] View1 : (%f,%f,%f)"), anatomicalView[0], anatomicalView[1], anatomicalView[2] );
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] View2 : (%f,%f,%f)"), anatomicalView[3], anatomicalView[4], anatomicalView[5] );
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] View3 : (%f,%f,%f)"), anatomicalView[6], anatomicalView[7], anatomicalView[8] );
	...
	width = dimensions[0]; 
	height = dimensions[1];
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] dimensions: x=%d, y=%d, z=%d"), dimensions[0], dimensions[1], dimensions[2]);

	double spacing[3];
	cutter->GetSpacing(spacing);  
	sizeX = spacing[0]; 
	sizeY = spacing[1]; 
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] spacing: x=%.2f, y=%.2f, z=%.2f"), spacing[0], spacing[1], spacing[2]);
	...
	//FString filename = FString::Printf(key.c_str());
	UE_LOG(LogTemp, Warning, TEXT("[CreateBuffer] length : %d "), length);
	return buffer; 
}
