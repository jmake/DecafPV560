...
//----------------------------------------------------------------------------------|  |--//
//--------------------------------------------------------------------------------| UE |--//
TArray<double> GenerateChessboardData(int32 W, int32 H, int32 NumSquaresX, int32 NumSquaresY)
{
    TArray<double> Data;
...
void CenterMeshAt(UProceduralMeshComponent* mesh, const FVector& center)
{
    ...
    // Compute translation needed to center the mesh at 'center'
    FVector Offset = center - MeshCenter;

    // Apply new relative location
...
void AMyActor1::BeginPlay()
{
	...
	TArray<double> Data = GenerateChessboardData(width, height, NumSquaresX, NumSquaresY); 
	//TexturecreateTest(Data, width, height, GetWorld()); 

	TArray<float> data = VtiTest(width, height); 	
	UE_LOG(LogTemp, Warning, TEXT("[VtiTest] width : %d, height : %d, Num : %d"), width, height, data.Num() );

    for (int32 i = 0; i < data.Num(); i++) double var = data[i]; 
	TexturecreateTest(data, width, height, GetWorld()); 
} 
