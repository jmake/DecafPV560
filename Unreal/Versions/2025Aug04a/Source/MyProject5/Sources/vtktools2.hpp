...
std::cout <<"- X : " <<  right[0] <<" " <<  right[1] <<" " <<  right[2] <<" \n"; 
std::cout <<"- Y : " <<  newUp[0] <<" " <<  newUp[1] <<" " <<  newUp[2] <<" \n"; 
std::cout <<"- Z : " <<      n[0] <<" " <<      n[1] <<" " <<      n[2] <<" \n"; 
...
//--------------------------------------------------------------------------||--//
vtkDataObject* CutterPlane4(
    vtkDataObject* obj,
    const std::vector<double>& right,     // X axis 
    const std::vector<double>& up,        // Y axis 
    const std::vector<double>& normal,    // Z axis 
...
auto start = std::chrono::high_resolution_clock::now();
    ...
    vtkImageData* vti = static_cast<vtkImageData*>(obj);
    if (!vti) {
        std::cerr << "[CutterPlane3] Error: input is not vtkImageData.\n";
        return nullptr;
    }

    if (right.size() != 3 || up.size() != 3 || normal.size() != 3 || origin.size() != 3) {
        std::cerr << "[CutterPlane3] Error: All input vectors must be of size 3.\n";
        return nullptr;
    }

    // Copy and normalize input vectors
    ...
    vtkSmartPointer<vtkMatrix4x4> axes = vtkSmartPointer<vtkMatrix4x4>::New();
    for (int i = 0; i < 3; ++i) {
        axes->SetElement(i, 0, r[i]);        // X axis
        axes->SetElement(i, 1, u[i]);        // Y axis
        axes->SetElement(i, 2, n[i]);        // Z axis (normal)
        axes->SetElement(i, 3, origin[i]);   // Origin
    }
    ...
    // Project corners to compute output slice size
    double minU = 1e12, maxU = -1e12;
    double minV = 1e12, maxV = -1e12;
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        double uVal = vtkMath::Dot(vec, r);
        double vVal = vtkMath::Dot(vec, u);

        minU = std::min(minU, uVal);
        maxU = std::max(maxU, uVal);
...
auto end = std::chrono::high_resolution_clock::now();
auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
std::cout << "\t - [CutterPlane3] Execution time: " << duration.count() / 1000.0 << " sec" << std::endl;

    return CenterAtOrigin( reslice->GetOutputDataObject(0) ); 
    //return reslice->GetOutputDataObject(0);
}
...
void PrintPointDataArrays(vtkPointData* pointData) 
{
  ...
  for (int i = 0; i < numArrays; ++i) 
  {
      ...
      for(int k=0; k< ncols; k++) // Tuples
      {
          std::vector<double> rows; 
          for(int j=0; j< nrows; j++) // Components
          {
              auto value = array->GetComponent(k,j); 
...
class ExtractorSlicer  
{
    ...
    {
      vtkDataObject *cutter = CutterPlane1(vti, o, normal); 
