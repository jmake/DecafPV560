...
UpdateDynamicTexture2D(dt, dataTotal1, ColorMin, ColorMax);
SaveDynamicTexture2D(dt, dynamicTextureName + "_C1.png"); 

	int total = widthLocal * heightLocal * deepLocal; 
	UE_LOG(LogTemp, Warning, TEXT("[UpdateDymanicTexture] (width,height,deep) : (%d,%d,%d) -> %d "), widthLocal, heightLocal, deepLocal, total);
}
