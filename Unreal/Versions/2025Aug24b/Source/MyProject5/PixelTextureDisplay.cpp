...
APixelTextureDisplay::APixelTextureDisplay()
{
    ...
    static ConstructorHelpers::FObjectFinder<UMaterialInterface> Mat(TEXT("/Game/SpicyTech/M_RuntimeTexture"));
    BaseMaterial = Mat.Object; 
}


void APixelTextureDisplay::Init(int32 InWidth, int32 InHeight, float sizex, float sizey)
{
    Width = InWidth;
    Height = InHeight;
    ...
    PlaneComponent->SetWorldScale3D( FVector(sizex, sizey, 1.0) );
...
void APixelTextureDisplay::Apply(TArrayView<float> Data, float ScaleFactor) 
{
...
void APixelTextureDisplay::Save(std::string fname)
{
    FString OutputPath = FPaths::ProjectSavedDir() / UTF8_TO_TCHAR(fname.c_str()); 
    SaveTextureToPNG(DynamicTexture, OutputPath);

    UE_LOG(LogTemp, Warning, TEXT("[APixelTextureDisplay] Texture saved to: %s"), *OutputPath);
}
...
void APixelTextureDisplay::UpdateActorTransformIfChanged()
{
    ...
    UE_LOG(LogTemp, Warning, TEXT("[APixelTextureDisplay] Actor rotation: Pitch=%f Yaw=%f Roll=%f"), ActorRotation.Pitch, ActorRotation.Yaw, ActorRotation.Roll);
}
