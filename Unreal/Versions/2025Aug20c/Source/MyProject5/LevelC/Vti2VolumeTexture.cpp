...
)
{
    ...
    if (Data.Num() != Width * Height * Depth) 
	{
		return ;
	}
	...
	//FLinearColor ColorMin(1.0f, 0.0f, 0.0f, 0.01f); ; //FLinearColor::Red;
	//FLinearColor ColorMax = FLinearColor::Blue;	

    // Step 2: create transient volume texture
    //UVolumeTexture* VolumeTex = UVolumeTexture::CreateTransient(Width, Height, Depth, PF_B8G8R8A8, NAME_None);
	if (VolumeTex)
	{
...
TArray<float> ReorderedData;
ReorderedData.SetNumUninitialized(Width * Height * Depth);

for (int32 z = 0; z < Depth; ++z)
{
    int32 tileX = z % tilesX;
    int32 tileY = z / tilesX;

    for (int32 y = 0; y < Height; ++y)
    {
        for (int32 x = 0; x < Width; ++x)
        {
            int32 globalX = tileX * Width + x;
            int32 globalY = tileY * Height + ((Height - 1) - y);

            int32 widthTotal = Width * tilesX;
            int32 srcIndex = globalY * widthTotal + globalX;
            int32 dstIndex = z * Width * Height + y * Width + x;

            ReorderedData[dstIndex] = Data[srcIndex];
        }
    }
}
...
/*
    ...
    int32 globalX = tileX * Width + X;
    int32 globalY = tileY * Height + ((Height - 1) - Y); // flip Y

    int32 widthTotal = Width * tilesX;
    int32 Index = globalY * widthTotal + globalX;
*/
...
//float Val = ReorderedData[Index];

        // Normalize
        float Normalized = FMath::GetMappedRangeValueClamped(FVector2D(Min, Max), FVector2D(0.f, 1.f), Val);

        // Color mapping
        FColor Color = FMath::Lerp(ColorMin, ColorMax, Normalized).ToFColor(false);

        // Write BGRA (PixelSize)
        Dest[0] = Color.B;
        Dest[1] = Color.G;
        Dest[2] = Color.R;
        Dest[3] = Color.A;
...
/*
	// Avoid the change to PF_DXT1 even though you asked for PF_B8G8R8A8 (Unreal applies compression, DXT/BC formats)
	VolumeTex->CompressionNone = true;
	VolumeTex->CompressionSettings = TC_VectorDisplacementmap; // keeps full precision
...
SaveTArrayUint8(VolumeBuffer, widthTotal, heightTotal, dynamicTextureName + "_2.png"); 

    UE_LOG(LogTemp, Warning, TEXT("[CreateVolumeTextureFromFloatData] Volume texture created directly from float data!"));
    //return VolumeTex;
}
...
//----------------------------------------------------------------------------------|  |--//
TArray<float> DataTiles2( // depth-major linear layout
    TArray<float>& dataTotal,
    int z,
    TArray<float> dataLocal,
    int widthLocal,
...
//----------------------------------------------------------------------------------|  |--//
TArray<float> CreateDataTest(TArray<float>& data, int width, int height)
{
	//TArray<float> data; 
	data.Empty();  
...
void AVti2VolumeTexture::CreateMeshComponent1(UTexture2D* dynamicTexture)
{
	...
	UStaticMeshComponent* planeComponent = CreateMeshComponent(this, planePath, FVector(1.0), FVector(-350.0f,-50.0f,110.0f), FRotator(90.0f,.0f,0.0f)); 
...
/*
	...
	CreateMeshComponent1(dynamicTexture); 
	...
	{ 
		TArray<float> dataLocal; //dataLocal.Init(i, widthLocal * heightLocal);
		CreateDataTest(dataLocal, widthLocal, heightLocal); 
		//DataTiles1(dataTotal, i, dataLocal, widthLocal, heightLocal, widthTotal, heightTotal);
		DataTiles2(dataTotal, i, dataLocal, widthLocal, heightLocal, widthTotal, heightTotal);
	}

	UpdateDynamicTexture2D(dynamicTexture, dataTotal);
	SaveDynamicTexture2D(dynamicTexture, dynamicTextureName + "_1.png"); 

	UVolumeTexture* volumeTexture = CreateVolumeTextureFromFloatData2(
		widthLocal, heightLocal, tilesX, tilesY,
		ColorMin, ColorMax,  
		TArrayView<float>(dataTotal));	

	CreateMeshComponent2(volumeTexture); 
*/
}
...
if (MeshComponent3)
{
	...
	int total = widthLocal * heightLocal * deepLocal; 
	UE_LOG(LogTemp, Error, TEXT("[UpdateDymanicTexture] (width,height,deep) : (%d,%d,%d) -> %d "), widthLocal, heightLocal, deepLocal, total);
}
